...
有多普通
![](../../💼背包/📎附件/Pasted%20image%2020230331182509.png)
这个神经网络，可以轻松解决普通的线性二分类问题
如下
![](../../💼背包/📎附件/Pasted%20image%2020230331182541.png)
So easy

可是现实生活中，更多的是这种复杂的二分类，你会发现，一个普通的线性神经网络无论怎么移动和反转，都无法将上图分类，因为给出的样本点就不是线性可分的
...
>通过不断叠加简单神经网络得到一个复杂神经网络，最后这个复杂的神经网络可以将上述圆圈和三角形分类

那我们画一个复杂的
![](../../💼背包/📎附件/Pasted%20image%2020230331182639.png)
```
...
## 第一个问题(ﾟ▽ﾟ*) 线性神经网络没法解决非线性问题

线性神经网络没法解决非线性问题

那我们就换种思路：
我们能不能把线性神经网络，转成非线性的，也就是把分隔直线转换成曲线。
...
![](../../💼背包/📎附件/Pasted%20image%2020230331182929.png)
Y = W*X*X+B 经过一系列运算（理想的）

我们得到了如下：
![](../../💼背包/📎附件/Pasted%20image%2020230331183005.png)
变成曲线后，好像可以把这两类区分开诶

## 第二个问题(ﾟ▽ﾟ*) 需要一个函数可以对得到的Y值进行分类
...
意味着gradient（梯度）和X 无关了，也就是很输入无关了

我们在做反向传播的时候，Gradient 的改变也变成常数，和输入的改变X(delta(x)) 无关了

## 综合以上四个问题ヽ(￣▽￣)ﾉ激活函数（Activation Function）

这个时候，就是我们激活函数（Activation Function）上场了

因为我们最终要解决一个分类问题（classification）
那我们还是从最简单的二分类非线性激活函数开始---阶跃函数（Step Function）
![](../../💼背包/📎附件/Pasted%20image%2020230331183214.png)
当输入（也就是神经网络Y的输出）大于0就被分类到1（100% 被激活 ），小于0就分到0，（没有被激活）

可是激活值只有100%或者0%未免太不负责了，我们希望它可以是0%---100%任意值
值越大，激活程度越高 对于分类，也就意味着它属于这一类的概率越大

综上所述
各种激活函数层数不穷，各有优缺点，比如大家都经常听到或者用到的，Relu, sigmoid,tanh等等
![](../../💼背包/📎附件/Pasted%20image%2020230331183256.png)
